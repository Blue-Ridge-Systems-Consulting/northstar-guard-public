#!/bin/zsh
# Install the tested Northstar Guard macOS implementation as native Apple Silicon arm64.
set -euo pipefail
...
case "${1:-}" in
  ...
  --reports-dir) [[ -n "${2:-}" ]] || { print -u2 "Usage: $0 --reports-dir DIRECTORY"; exit 64; }; REPORTS_DIR="$2" ;;
  *) print -u2 "Usage: $0 [--upgrade|--repair|--status|--uninstall|--reports-dir DIRECTORY]"; exit 64 ;;
esac

[[ "$(uname -m)" == "arm64" ]] || { print -u2 "This package requires an Apple Silicon Mac (arm64)."; exit 1; }
command -v clang >/dev/null || { print -u2 "clang is required. Install Apple Command Line Tools first: xcode-select --install"; exit 1; }
...
if [[ "$ACTION" == "uninstall" ]]; then
  ...
  rm -f "$PLIST"
  print "Northstar Guard service removed; reports and configuration were retained at $SUPPORT_DIR."
  exit 0
fi
...
"$SCRIPT_DIR/build.sh"
install -m 755 "$SCRIPT_DIR/bin/northstar-guard" "$SUPPORT_DIR/northstar-guard"
install -m 755 "$SCRIPT_DIR/bin/NorthstarGuardLauncher" "$APP_DIR/Contents/MacOS/NorthstarGuardLauncher"
...
[[ -f "$SUPPORT_DIR/config.json" ]] || printf '{\n  "reportsDirectory": "%s"\n}\n' "$REPORTS_DIR" > "$SUPPORT_DIR/config.json"
[[ -f "$SUPPORT_DIR/control.json" ]] || printf '{\n  "monitoringEnabled": true\n}\n' > "$SUPPORT_DIR/control.json"
print -r -- "$PACKAGE_VERSION" > "$SUPPORT_DIR/agent-version"
[[ -f "$SUPPORT_DIR/agent-id" ]] || print -r -- "mac-$(scutil --get ComputerName 2>/dev/null | tr '[:upper:] ' '[:lower:]-')-$(uuidgen | tr -d '-' | cut -c1-12)" > "$SUPPORT_DIR/agent-id"
...
launchctl bootstrap "gui/${UID_VALUE}" "$PLIST"
open "$APP_DIR"
print "Northstar Guard Apple Silicon ${ACTION} complete (v${PACKAGE_VERSION})."
